"""
Simple test for Stagnation Recovery System
"""
...
# Add autonomy directory to path
sys.path.insert(0, os.path.abspath('tiannara_core/autonomy'))

from stagnation_detector import StagnationDetector, StagnationSeverity
...
def test_cooldown_period():
    ...
    current_ep = detector.episode_numbers[-1] if detector.episode_numbers else 0
    print(f"First recovery at episode {current_ep}: {action1.strategy.value}")
    ...
    current_ep = detector.episode_numbers[-1] if detector.episode_numbers else 0
    print(f"Third recovery at episode {current_ep}: {action3.strategy.value}")
...
def test_recovery_statistics():
    ...
    detector = StagnationDetector(window_size=20, threshold=0.01)
    config = {'min_episodes_between_recovery': 5}  # Short cooldown for testing
    recovery = StagnationRecovery(detector, config=config)
    ...
    for improvement, description in scenarios:
        # Create stagnation (need enough episodes between recoveries)
        for i in range(30):
            episode_counter += 1
            detector.record_episode(episode_counter, 0.6, 0.5)
        ...
        if action:
            ...
            print(f"  {description}: No recovery applied (cooldown)")
    ...
    assert final_stats['total_recoveries'] > 0, "Should have at least some recoveries"
