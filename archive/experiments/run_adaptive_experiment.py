"""Run experiment with adaptive difficulty on algorithm domain."""
import sys
...
def run_adaptive_experiment(num_episodes=100):
    """Run experiment with adaptive difficulty scaling."""
    ...
    print("=" * 80)
    print("ADAPTIVE DIFFICULTY EXPERIMENT - ALGORITHM DOMAIN")
    print("=" * 80)
    print(f"Task Types: {task_gen.task_types}")
    print(f"Episodes: {num_episodes}")
    print(f"Starting Quality: {evolver.quality_level:.2f}")
    print(f"Difficulty Schedule: Easy (0-30) -> Medium (31-70) -> Hard (71-100)")
    print("=" * 80)
    
    # Open log file
    log_path = Path(__file__).parent / "logs" / "adaptive_algorithm_episodes.jsonl"
    ...
    for episode in range(num_episodes):
        # Generate task with episode number for adaptive difficulty
        ...
        # Update task generator with performance (for adaptive difficulty)
        ...
        if (episode + 1) % 10 == 0 or episode == 0:
            ...
            print(f"Episode {episode+1:3d}/{num_episodes}: "
                  ...
                  f"Skills={len(evolver.skill_library)}, Difficulty={task_gen.difficulty_level}")
    
    elapsed = time.time() - start_time
    
    # Final analysis
    print("\n" + "=" * 80)
    print("FINAL RESULTS - ADAPTIVE DIFFICULTY")
    print("=" * 80)
    ...
    print("-" * 80)
    for diff_level, episodes in difficulty_stats.items():
        if episodes:
            avg_score = sum(e["score"] for e in episodes) / len(episodes)
            success_rate = sum(1 for e in episodes if e["success"]) / len(episodes) * 100
            print(f"{diff_level.upper():10s}: {len(episodes):3d} episodes, "
