...
\> Don’t build “a secure system.” Build a system that continuously learns attacker behavior, mutates defenses, validates survivability, and evolves security policies over time.
...
{  
  "target": "sandbox\_executor",  
  "attack\_type": "prompt\_injection",  
  "constraints": {  
    "stealth": true,  
    "max\_tokens": 500  
  }  
...
{  
  "attack\_trace\_id": "atk\_991",  
  "payload": "...",  
  "mutation\_lineage": \["base\_prompt\_injection", "context\_hijack\_v3"\],  
  "severity\_prediction": 0.81  
}
...
{  
  ...
  "events": \[  
    {  
      "step": 1,  
      "action": "prompt\_override",  
      "effect": "memory\_access\_attempt"  
    },  
    {  
      "step": 2,  
      "action": "tool\_call\_hijack",  
      "effect": "sandbox\_escape\_attempt"  
    }  
  \]  
}
...
Over time: Tiannara recognizes patterns before full attacks happen.
...
Goal: See if Tiannara recognizes intent rather than exact phrasing.
...
Agent B: Uses them later to manipulate retrieval.
...
Otherwise attackers target: the evolution process itself.
