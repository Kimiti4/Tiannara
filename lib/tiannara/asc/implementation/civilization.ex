...
defmodule Tiannara.ASC.Implementation.Civilization do
  ...
  alias Tiannara.ASC.ProjectWorld.{Capability, TestContract}
  ...
  defp generate_source_files(%ProjectWorld{} = world, project_id) do
    prefix = project_module_prefix(project_id)

    world.capabilities
    |> Enum.group_by(&module_group/1)
    |> Enum.map(fn {group, caps} ->
      module_name = "#{prefix}.#{group}"
      content     = build_module(module_name, caps, world)
      ...
      %{
        ...
        content:      content,
        capabilities: Enum.map(caps, & &1.name),
  ...
  defp build_module(module_name, capabilities, world) do
    cap_list  = Enum.map_join(capabilities, "\n", fn cap -> "    - #{cap.name}" end)
    functions = Enum.map_join(capabilities, "\n\n", fn cap ->
      contracts = Enum.filter(world.test_contracts, &(&1.source_ref == cap.id))
      build_function(cap, contracts)
    end)
  ...
  defp build_function(%Capability{} = cap, contracts) do
    fn_name  = cap.verb |> String.downcase() |> String.replace(~r/\s+/, "_")
    subject  = cap.subject |> String.downcase() |> String.replace(~r/\s+/, "_")
    full_fn  = "#{fn_name}_#{subject}"
    ...
    pre  = if cap.preconditions  != [], do: "  Preconditions:  #{Enum.join(cap.preconditions, ", ")}\n", else: ""
    post = if cap.postconditions != [], do: "  Postconditions: #{Enum.join(cap.postconditions, ", ")}\n", else: ""

    """
      @doc \"\"\"
      #{String.capitalize(cap.name)}.

    #{pre}#{post}  TestContracts: #{length(contracts)}
      \"\"\"
      @spec #{full_fn}(map()) :: {:ok, term()} | {:error, term()}
      def #{full_fn}(params) do
        #{contract_comments}
  ...
  defp module_group(%Capability{subject: subject}) do
    subject
    |> String.split()
    |> List.first("Entity")
    |> String.capitalize()
