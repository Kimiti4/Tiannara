defmodule TiannaraRuntime.Cognitive.Reflection.BiasAssessment do
  defstruct [:id, :session_id, :bias_type, :severity, :confidence, :dimensions, :evidence, :recommendation, :fingerprint, :schema_version, :ontology_version, :created_with_phase]
  ...
  def new(fields) do
    ...
    struct = %__MODULE__{
      ...
      bias_type: Map.get(fields, :bias_type),
      severity: Map.get(fields, :severity, 0.0),
      confidence: Map.get(fields, :confidence, 0.0),
      dimensions: Map.get(fields, :dimensions, %{}),
      evidence: Map.get(fields, :evidence, []),
      recommendation: Map.get(fields, :recommendation),
