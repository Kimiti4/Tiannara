defmodule TiannaraRuntime.Cognitive.Decision.DecisionPolicy do
  defstruct [:id, :name, :scope, :priority, :constraints, :status, :fingerprint, :schema_version, :ontology_version, :created_with_phase, :migration_version]
  ...
  def new(fields) do
    ...
    struct = %__MODULE__{
      id: id, name: fields[:name], scope: fields[:scope],
      priority: fields[:priority] || 0, constraints: fields[:constraints] || [],
  ...
  def generate_id(fields) do
    base = "#{fields[:name]}_#{:erlang.unique_integer([:positive])}"
  ...
  def validate(%__MODULE__{} = s) do
    ...
    errors = if is_nil(s.name), do: [{:name, :required} | errors], else: errors
