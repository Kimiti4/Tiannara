defmodule TiannaraRuntime.Cognitive.Decision.Decision do
  defstruct [:id, :session_id, :candidates, :selected, :status, :fingerprint, :schema_version, :ontology_version, :created_with_phase, :migration_version]
  ...
  def new(fields) do
    ...
    struct = %__MODULE__{
      id: id, session_id: fields[:session_id], candidates: fields[:candidates] || [],
      selected: fields[:selected], status: fields[:status] || :pending,
