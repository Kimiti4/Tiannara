defmodule TiannaraRuntime.Cognitive.Planning.PlanningArchaeology do
  defstruct [:id, :session_id, :origin, :lineage, :purpose, :fingerprint, :schema_version, :ontology_version, :created_with_phase, :migration_version]
  ...
  def generate_id(fields) do
    base = "#{fields.session_id}_#{fields.origin}_#{:erlang.unique_integer([:positive])}"
  ...
  def validate(%__MODULE__{} = s) do
    ...
    errors = if is_nil(s.session_id), do: [{:session_id, :required} | errors], else: errors
    errors = if is_nil(s.origin), do: [{:origin, :required} | errors], else: errors
