# scripts/rea_7_g_0_civilization_ignition.exs
defmodule Tiannara.REA.CivilizationIgnitionTest do
  @moduledoc """
  Phase REA-7G0: Civilization Ignition
  
  Tests if civilization requires a developmental phase to form.
  Introduces three phases: Ignition, Consolidation, and Strict Civilization,
  bridging the gap between spontaneous coordination and strict dependency.
  """
  require Logger
  ...
  def run() do
    Logger.info("🔥 [REA-7G0] Initiating Civilization Ignition Test...")
  ...
  defp evolve([], kg, epochs_remaining, registry, telemetry, state) do
    Logger.warning("☠️ TOTAL EXTINCTION at Epoch #{@epochs - epochs_remaining}. Ignition failed.")
  ...
  defp evolve(pop, kg, epochs_remaining, registry, telemetry, state) do
    ...
    # Reservoir wipe at Epoch 1500
    state = if current_epoch == 1500 do
    ...
    {regenerated_kg, dividend_ledger, deep_energy_gen, mcr_nodes, isr_nodes, crr_reactivated} = process_nodes(updated_kg, current_epoch)
    ...
    isr = isr_nodes / 100.0
    total_energy = deep_energy_gen + epoch_metrics.shallow_energy_gen
    cdr = if total_energy > 0, do: deep_energy_gen / total_energy, else: 0.0
    cm = mcr_nodes - epoch_metrics.state.prev_mcr_nodes
    
    new_telemetry = if epochs_remaining == 1 or rem(current_epoch, 100) == 0 do
      [record_snapshot(next_gen, current_epoch, final_registry, cr, mcr, isr, cdr, cm, crr_reactivated, final_state_with_counts) | telemetry]
    else
      telemetry
    end
  ...
  defp process_nodes(kg, current_epoch) do
  ...
  defp record_snapshot(pop, epoch, registry, cr, mcr, isr, cdr, cm, crr, state) do
    pop_size = length(pop)
    ...
    %{
      ...
      cr: cr, mcr: mcr, isr: isr, cdr: cdr, cm: cm, crr: crr, max_share: max_share,
  ...
  defp print_results(telemetry) do
    Logger.info("\n================================================================================================================================")
    Logger.info("🔥 REA-7G0 CIVILIZATION IGNITION MATRIX (Phased Dependency)")
    Logger.info("================================================================================================================================")
    Logger.info("Epoch | Pop | Spc | MCR   | ISR   | CDR   | CM  | CRR | Max Niche | %Comp | %Expl | %Pred | Comp | Expl | Pred | Gen")
    Logger.info("--------------------------------------------------------------------------------------------------------------------------------")
    ...
    Enum.each(Enum.reverse(telemetry), fn r ->
      ...
      cdr = Float.round(r.cdr, 3) |> Float.to_string() |> String.pad_trailing(5)
      cm  = "#{r.cm}" |> String.pad_trailing(3)
      ...
      nc = r.niche_census
      Logger.info("#{e} | #{p} | #{spc} | #{mcr} | #{isr} | #{cdr} | #{cm} | #{crr} | #{m_sh} | #{c_pct} | #{e_pct} | #{p_pct} | #{nc.comp}   | #{nc.expl}   | #{nc.pred}   | #{nc.gen}")
    end)
    Logger.info("================================================================================================================================")
  end
...
Tiannara.REA.CivilizationIgnitionTest.run()
