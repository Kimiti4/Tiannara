...
class SimulatorState:
    def __init__(self, pos, trust_matrix, S_sparse, niche, agent_consumed_amt, active_triplets):
        ...
        self.S_sparse = S_sparse  # List of 3 csr_matrices of shape (N_NODES, N_AGENTS)
...
def extract_features(state, ec_matrix, cp_matrix, target_node, lineage_age, prev_target, radius=5):
    ...
    for t in range(3):
        institution_strength += state.S_sparse[t][left:right, :].sum()
    institution_strength /= max(1, (right-left))
    ...
    x_inst = np.array([local_dcr, local_triplets, lineage_age, institution_strength, ec_flow, cp_flow, migration_vel], dtype=np.float32)
    ...
    geo_density = len(agents_in_radius)
    geo_field_vol = institution_strength
    geo_mig_vel = migration_vel
    ...
    x_geo = np.array([geo_density, geo_field_vol, geo_mig_vel, density_grad], dtype=np.float32)
    x_rand = np.random.rand(7).astype(np.float32)
    
    return x_micro, x_inst, x_rand, x_geo

def run_ternary_hazard_tournament():
    np.random.seed(42)
    ...
    print("🌌 [REA-7N-B2] Booting Ternary Hazard Prediction Tournament (Fast CSR Batch Engine)")
    print(f"Parameters: N_NODES={N_NODES}, N_AGENTS={N_AGENTS}, WALK_STEPS={WALK_STEPS}, RADIUS={RADIUS}")
