...
def get_basin_metrics(state, target_node, radius=5):
    ...
    trust_centrality = np.zeros(N_AGENTS, dtype=np.float32)
...
def run_simulation_step_c(state, neighbors, K_csr, target_trace, emit_trace, MAX_PHI, DECAY, MORTALITY_RATE, WALK_STEPS, world_mode='A'):
    ...
    return CE
def original_run_simulation_step_c(state, neighbors, K_csr, target_trace, emit_trace, MAX_PHI, DECAY, MORTALITY_RATE, WALK_STEPS, world_mode='A'):
    N_AGENTS = len(state.pos)
    ...
    for step in range(WALK_STEPS):
        ...
        # Precompute trust to avoid massive O(N) penalty in the inner loop
        local_trust_all = np.sum(state.trust_matrix > 1.5, axis=1)
        ...
        for a in range(N_AGENTS):
            ...
            macro_phi = np.zeros_like(phi)
            if world_mode in ['B', 'C'] and w_macro > 0.1:
                # Spatial Regulation (Safety and Congestion)
                safety_node = state.target_node if world_mode == 'B' else np.random.randint(0, N_NODES)
                # Distance to safety node
                dists = np.abs(c_nodes - safety_node)
                dists = np.minimum(dists, N_NODES - dists)
                macro_phi += 5.0 / (dists + 1.0)
...
def apply_topological_shock(state, target_node, radius=5):
    ...
    if len(agents_in_radius) > 0:
        ...
        # Remove random edges to shock redundancy
...
def run_world_c_tournament():
    ...
    print("🌌 [REA-7N-C] Booting Cybernetic Control Tournament")
    ...
    dcr, dci_t, r_t, scp, _, c = get_basin_metrics(state, state.target_node)
    tfi_t = compute_tfi(dcr, scp, dci_t, r_t, c, state, state.target_node)
    
    state.DCI_target = dci_t
    state.R_target = r_t * 1.10 # +10% optimal redundancy
    state.TFI_target = tfi_t
    
    print(f"\n[Cybernetic Attractor Locked]")
    print(f"  DCI_target = {state.DCI_target:.3f}")
    print(f"  R_target   = {state.R_target:.2f}")
    print(f"  TFI_target = {state.TFI_target:.1f}")
    
    print("\nPhase 2: Applying Deep Topological Shock (Epoch 400)")
    state_shocked = apply_topological_shock(state, state.target_node)
    
    dcr_s, dci_s, r_s, scp_s, _, c_s = get_basin_metrics(state_shocked, state_shocked.target_node)
    tfi_s = compute_tfi(dcr_s, scp_s, dci_s, r_s, c_s, state_shocked, state_shocked.target_node)
    print(f"  Post-Shock TFI dropped to {tfi_s:.1f}")
    ...
    metrics = {
        'A': {'tfi': [], 'ce': 0.0, 'rec': -1, 'overshoot': 0.0},
        'B': {'tfi': [], 'ce': 0.0, 'rec': -1, 'overshoot': 0.0},
        'C': {'tfi': [], 'ce': 0.0, 'rec': -1, 'overshoot': 0.0}
    }
    
    for epoch in range(401, 801):
        for mode, st in zip(['A', 'B', 'C'], [state_A, state_B, state_C]):
            S_sum_A = np.array(st.S_sparse[0].sum(axis=1) + st.S_sparse[1].sum(axis=1) + st.S_sparse[2].sum(axis=1)).flatten()
            st.target_node = int(np.argmax(S_sum_A))
            ce = run_simulation_step_c(st, neighbors, K_csr, target_trace, emit_trace, MAX_PHI, DECAY, MORTALITY_RATE, WALK_STEPS, world_mode=mode)
            metrics[mode]['ce'] += ce
            
            dcr_a, dci_a, r_a, scp_a, _, c_a = get_basin_metrics(st, st.target_node)
            tfi_A = compute_tfi(dcr_a, scp_a, dci_a, r_a, c_a, st, st.target_node)
            metrics[mode]['tfi'].append(tfi_A)
            
            error_magnitude = abs(tfi_A - state.TFI_target)
            if error_magnitude > metrics[mode]['overshoot']:
                metrics[mode]['overshoot'] = error_magnitude
            
            if metrics[mode]['rec'] == -1 and tfi_A >= 0.90 * state.TFI_target:
                metrics[mode]['rec'] = epoch - 400
                
        if epoch % 100 == 0:
            print(f"  Epoch {epoch} | TFI -> A: {metrics['A']['tfi'][-1]:.1f} | B: {metrics['B']['tfi'][-1]:.1f} | C: {metrics['C']['tfi'][-1]:.1f}")
            print(f"            | CE  -> A: {metrics['A']['ce']:.0f} | B: {metrics['B']['ce']:.0f} | C: {metrics['C']['ce']:.0f}")
            
    print("\n========================================================")
    print("REA-7N-C Homeostatic Topological Control Results")
    print("========================================================")
    print(f"Target Equilibrium TFI: {state.TFI_target:.1f}")
    
    for mode in ['A', 'B', 'C']:
        name = "World A (Micro Only)" if mode == 'A' else ("World B (Regulation)" if mode == 'B' else "World C (Random Reg)")
        surv = 0 if metrics[mode]['tfi'][-1] < 1.0 else 2
        rec_str = 'Never Recovered' if metrics[mode]['rec'] == -1 else f"{metrics[mode]['rec']} epochs"
        print(f"\n[{name}]")
        print(f"  Final TFI: {metrics[mode]['tfi'][-1]:.1f}")
        print(f"  Survival:  {surv}")
        print(f"  Topology Recovery Half-Life (TRH): {rec_str}")
        print(f"  Total Control Effort (CE): {metrics[mode]['ce']:.0f}")
        print(f"  Max Overshoot |TFI - Target|: {metrics[mode]['overshoot']:.1f}")

if __name__ == "__main__":
    run_world_c_tournament()
