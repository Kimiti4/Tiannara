...
def run_condition(alpha):
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        # --- REA-7K: Network Integrity Ratio (LCC) ---
        ...
        for a in dead_agents:
            ...
            # Inheritance Scalar
            trust_matrix[a, :] = np.maximum(1.0, trust_matrix[a, :] * alpha)
            partner_success_count[a, :] = (partner_success_count[a, :] * alpha).astype(np.int32)
    ...
    return alpha, deaths, ccr, rrt, scp, mean_lcc

if __name__ == "__main__":
    alphas = [0.0, 0.25, 0.50, 0.75, 1.0]
    print("🌌 [REA-7K Python Engine] Booting Multipass Sweep...")
    start = time.time()
    with multiprocessing.Pool(5) as pool:
        results = pool.map(run_condition, alphas)
        
    print(f"Sweep completed in {time.time() - start:.2f} seconds\n")
    print("="*65)
    print(f"{'Alpha':<6} | {'Deaths':<8} | {'CCR':<6} | {'RRT':<6} | {'SCP':<6} | {'Mean LCC':<10}")
    print("-" * 65)
    for res in results:
        alpha, deaths, ccr, rrt, scp, mean_lcc = res
        print(f"{alpha:<6.2f} | {deaths:<8} | {ccr:<6.3f} | {rrt:<6.1f} | {scp:<6.1f} | {mean_lcc:<10.1f}")
