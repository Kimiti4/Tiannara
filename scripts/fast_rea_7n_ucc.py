...
def calculate_fingerprint(pos, trust_matrix, niche, ec_matrix, cp_matrix, active_triplets, center_node, radius, n_nodes):
    ...
    return [float(triplet_density), float(ec_flow), float(cp_flow), float(niche_entropy), float(average_trust)]
...
async def run_simulation():
    ...
    try:
        ...
        print("⚠️ No NATS server found. Running in offline mode.")
        nc = None
    ...
    print(f"🌌 [REA UCC NATS Engine] Booting 2,000 Epoch Event-Sourced Extraction...")
    ...
    for epoch in range(1, N_EPOCHS + 1):
        ...
        if nc:
            for node in np.where(new_basins)[0]:
                ...
                payload = json.dumps({"event": "basin_birth", "node_id": int(node), "epoch": epoch, "fingerprint": fp}).encode()
                await nc.publish("tiannara.rea.basins", payload)
            
            update_mask = (basin_presence > 0) & (basin_presence % 50 == 0) & (basin_absence == 0)
            for node in np.where(update_mask)[0]:
                fp = calculate_fingerprint(pos, trust_matrix, niche, ec_matrix, cp_matrix, active_triplets, node, RADIUS, N_NODES)
                payload = json.dumps({"event": "basin_update", "node_id": int(node), "epoch": epoch, "fingerprint": fp}).encode()
                await nc.publish("tiannara.rea.basins", payload)
                
            for node in np.where(dead_basins_mask)[0]:
                payload = json.dumps({"event": "basin_death", "node_id": int(node), "epoch": epoch, "final_age": int(basin_presence[node])}).encode()
                await nc.publish("tiannara.rea.basins", payload)
        ...
        if epoch % 100 == 0:
            ...
            print(f"Epoch {epoch}: {active_count} active basins.", flush=True)
