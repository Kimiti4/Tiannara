defmodule Tiannara.ASC.Crucible.EvolutionPhase5B do
  @moduledoc """
  Phase 5B Validation Campaign - Transfer Adaptation Engine
  
  Purpose: Validate that ASC can adapt and transfer knowledge across projects.
  
  Configuration:
  - 10 projects × 20 generations = 200 project-generations
  - Focus on transfer adaptation metrics
  ...
  @project_configs [
    ...
    # Additional 5 projects for Phase 5B validation
  ...
  def run do
    IO.puts("\n" <> String.duplicate("=", 80))
    IO.puts("🧬 PHASE 5B VALIDATION CAMPAIGN - TRANSFER ADAPTATION ENGINE")
    IO.puts(String.duplicate("=", 80))
    IO.puts("Purpose: Validate cross-project knowledge transfer with adaptation")
    ...
    IO.puts("  ✅ Adapted Transfer Success Rate > 5%")
    IO.puts("  ✅ Transferability Score > 0.20")
    IO.puts(String.duplicate("=", 80) <> "\n")
    ...
    # Print final summary with Phase 5B exit criteria
    print_phase5b_summary(all_results)
  end
  ...
  defp run_generation(genome, project_id, generation) do
    ...
    # Repair failures (now includes Transfer Adaptation Engine)
  ...
  defp print_phase5b_summary(all_results) do
    IO.puts("\n" <> String.duplicate("=", 80))
    IO.puts("📊 PHASE 5B VALIDATION RESULTS - TRANSFER ADAPTATION")
    IO.puts(String.duplicate("=", 80))
    ...
    IO.puts("\n🌍 Transfer Adaptation Metrics (Phase 5B):")
    ...
    IO.puts("   Raw Transfer Attempts: #{reuse_metrics.raw_transfer_attempts}")
    IO.puts("   Raw Transfer Successes: #{reuse_metrics.raw_transfer_successes}")
    IO.puts("   Adapted Transfer Attempts: #{reuse_metrics.adapted_transfer_attempts}")
    IO.puts("   Adapted Transfer Successes: #{reuse_metrics.adapted_transfer_successes}")
    
    adapted_success_rate = 
      if reuse_metrics.adapted_transfer_attempts > 0 do
        reuse_metrics.adapted_transfer_successes / reuse_metrics.adapted_transfer_attempts
      else
        0.0
      end
    
    IO.puts("   Adapted Transfer Success Rate: #{Float.round(adapted_success_rate * 100, 1)}%")
    
    # Calculate transferability score
    transferability_score = 
      if reuse_metrics.transfer_count > 0 do
        reuse_metrics.transfer_success_rate
      else
        0.0
      end
    
    IO.puts("   Transferability Score: #{Float.round(transferability_score, 3)}")
    ...
    # Criterion 4: Adapted Transfer Success Rate > 5%
    transfer_adaptation_success = adapted_success_rate > 0.05
    IO.puts("   4. Adapted Transfer Success Rate > 5%: #{if transfer_adaptation_success, do: "✅ PASS", else: "❌ FAIL"} (#{Float.round(adapted_success_rate * 100, 1)}%)")
    
    # Criterion 5: Transferability Score > 0.20
    transferability_pass = transferability_score > 0.20
    IO.puts("   5. Transferability Score > 0.20: #{if transferability_pass, do: "✅ PASS", else: "❌ FAIL"} (#{Float.round(transferability_score, 3)})")
    
    IO.puts("\n" <> String.duplicate("-", 80))
    if repair_success and reuse_success and velocity_success and transfer_adaptation_success and transferability_pass do
      IO.puts("🎉 ALL EXIT CRITERIA MET!")
      IO.puts("ASC has crossed from Adaptive Engineering to Knowledge-Propagating Engineering Civilization")
    else
...
# Run the campaign
Tiannara.ASC.Crucible.EvolutionPhase5B.run()
