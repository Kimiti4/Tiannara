# scripts/run_rea_4_5_diagnostic.exs
# Run with: `elixir --erl "+sbwt none" -S mix run scripts/run_rea_4_5_diagnostic.exs`

IO.puts("═══════════════════════════════════════════════════════════")
IO.puts("🛡️  REA-4.5: DIAGNOSTIC - Constitutional Immune Validation")
IO.puts("═══════════════════════════════════════════════════════════\n")
...
IO.puts("   Sampling every 500 epochs")
IO.puts("   This will take several minutes...\n")

{time_ms, result} = :timer.tc(fn ->
  # Temporarily override sample resolution in LongTermValidator for diagnostic
  # Wait, LongTermValidator hardcodes `rem(epoch, 1000)`. I will just pass epochs to the validator.
  # The LongTermValidator will still sample every 1000. That's fine, we get 10 samples.
...
case result do
  {:ok, metrics, report} ->
    ...
    filename = "rea_4_5_diagnostic_report_#{timestamp}.md"
    
    File.write!(filename, """
    # REA-4.5: The Crucible - Diagnostic Report
