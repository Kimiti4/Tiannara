# scripts/rea_7_c_dependency_necessity.exs
defmodule Tiannara.REA.DependencyNecessityTest do
  @moduledoc """
  Phase REA-7C: Dependency Necessity Test
  
  Implements Obligate Dependency through Stigmergic Traces and Primordial Reservoirs.
  Specialization without trace exchange results in starvation.
  """
  require Logger
  ...
  def run() do
    Logger.info("⛓️ [REA-7C] Initiating Obligate Dependency Test...")
  ...
  defp evolve(pop, kg, epochs_remaining, registry, telemetry, state) do
    ...
    # 1. Foraging with Strict Resource Gating
    {foraged_pop, updated_kg, epoch_metrics} = Enum.reduce(shuffled_pop, {[], kg, %{req_trace: 0.0, act_trace: 0.0, res_consumed: 0.0, node_trace_consumed: 0.0, state: state}}, fn agent, {pop_acc, current_kg, metrics} ->
      {updated_agent, new_kg, run_metrics} = simulate_gated_walk(agent, current_kg, @walk_steps, metrics.state)
      
      new_metrics = %{
        req_trace: metrics.req_trace + run_metrics.req_trace,
        act_trace: metrics.act_trace + run_metrics.act_trace,
        res_consumed: metrics.res_consumed + run_metrics.res_consumed,
        node_trace_consumed: metrics.node_trace_consumed + run_metrics.node_trace_consumed,
    ...
    # 3. Resource Regeneration & Trace Decay (0.99 to prevent infinite buildup of dead traces)
    ...
    # 5. Speciation & Extinction Tracking
    ...
    {next_gen, final_registry} = Enum.flat_map_reduce(survivors, updated_registry, fn agent, reg_acc ->
      if agent.energy >= @reproduction_threshold do
        ...
        {final_child_lineage, new_reg} = if div > @speciation_threshold do
          ...
          c_data = %{
            ancestral_weights: child_weights, status: :candidate, start_epoch: current_epoch,
            parent_lineage: parent.lineage_id, max_population: 1
          }
    ...
    # 7. Telemetry & DCR / PDR Calculation
    dcr = if epoch_metrics.req_trace > 0, do: epoch_metrics.act_trace / epoch_metrics.req_trace, else: 1.0
    total_trace_consumed = epoch_metrics.res_consumed + epoch_metrics.node_trace_consumed
    pdr = if total_trace_consumed > 0, do: epoch_metrics.res_consumed / total_trace_consumed, else: 0.0
    
    new_telemetry = if epochs_remaining == 1 or rem(current_epoch, 100) == 0 do
      [record_snapshot(next_gen, current_epoch, final_registry, updated_counts, dcr, pdr, final_state_with_counts) | telemetry]
    else
      telemetry
    end
  ...
  defp generate_finite_knowledge_graph(size) do
    ...
    nodes = Enum.reduce(0..(size-1), %{}, fn i, acc ->
      ...
      Map.put(acc, i, %{
        ...
        current_energy: @node_max_energy, novelty_trace: 0.0, compression_trace: 0.0,
  ...
  defp simulate_gated_walk(agent, kg, steps, initial_state) do
    ...
    Enum.reduce(1..steps, {agent, kg, %{req_trace: 0.0, act_trace: 0.0, res_consumed: 0.0, node_trace_consumed: 0.0, state: initial_state}}, fn _, {current_agent, current_kg, m} ->
      current_node_id = start_node # Simplification for pure yield testing without full traversal logic tracking visited
      # Actually, let's keep a random neighbor walk
      target_node_id = Enum.random(current_kg.nodes[current_node_id].neighbors)
      node = current_kg.nodes[target_node_id]
      
      yield = min(node.current_energy, @harvest_cap)
      
      # 1. EXPLORATION (Novelty) - Always unlocked
      raw_expl_yield = yield * Enum.at(agent_vec, 1) * node.novelty_value * specialization_bonus
      node_energy_after_expl = node.current_energy - raw_expl_yield
      node_nov_trace = node.novelty_trace + raw_expl_yield
      
      # 2. COMPRESSION - Requires Novelty Trace
      ...
      # 3. PREDICTION - Requires Compression Trace
      ...
      total_energy_gained = raw_expl_yield + actual_comp_yield + actual_pred_yield
      ...
      new_m = %{
        req_trace: m.req_trace + raw_comp_yield + raw_pred_yield,
        act_trace: m.act_trace + actual_comp_yield + actual_pred_yield,
        res_consumed: m.res_consumed + c_res_consumed + p_res_consumed,
        node_trace_consumed: m.node_trace_consumed + c_node_consumed + p_node_consumed,
        state: %{m.state | novelty_reservoir: new_nov_res, compression_reservoir: new_comp_res}
      }
  ...
  defp record_snapshot(pop, epoch, registry, counts, dcr, pdr, state) do
    pop_size = length(pop)
    ...
    %{
      ...
      dcr: dcr, pdr: pdr, nov_res: state.novelty_reservoir, comp_res: state.compression_reservoir,
  ...
  defp print_results(telemetry) do
    Logger.info("\n=========================================================================================================")
    Logger.info("⛓️ REA-7C DEPENDENCY NECESSITY MATRIX (Obligate Trace Economy)")
    Logger.info("=========================================================================================================")
    Logger.info("Epoch | Pop | Shannon | ESR   | Spc | PDR   | DCR   | Nov Res | Comp Res | Comp | Expl | Pred | Gen")
    Logger.info("---------------------------------------------------------------------------------------------------------")
    ...
    Enum.each(Enum.reverse(telemetry), fn r ->
      ...
      esr = Float.round(r.esr, 3) |> Float.to_string() |> String.pad_trailing(5)
      spc = "#{r.active_species}" |> String.pad_trailing(3)
      pdr = Float.round(r.pdr, 3) |> Float.to_string() |> String.pad_trailing(5)
      dcr = Float.round(r.dcr, 3) |> Float.to_string() |> String.pad_trailing(5)
      
      nr = Float.round(r.nov_res, 1) |> Float.to_string() |> String.pad_trailing(7)
      cr = Float.round(r.comp_res, 1) |> Float.to_string() |> String.pad_trailing(8)
      
      nc = r.niche_census
      Logger.info("#{e} | #{p} | #{d} | #{esr} | #{spc} | #{pdr} | #{dcr} | #{nr} | #{cr} | #{nc.comp}    | #{nc.expl}    | #{nc.pred}    | #{nc.gen}")
    end)
    Logger.info("=========================================================================================================")
  end
...
Tiannara.REA.DependencyNecessityTest.run()
