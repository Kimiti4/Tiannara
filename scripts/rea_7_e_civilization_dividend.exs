# scripts/rea_7_e_civilization_dividend.exs
defmodule Tiannara.REA.CivilizationDividendTest do
  @moduledoc """
  Phase REA-7E: Civilization Dividend (The Eden Mechanics)
  
  Tests if civilization becomes an attractor when cooperation unlocks
  permanent thermodynamic infrastructure (Eden Nodes) inaccessible to isolated agents.
  """
  require Logger
  ...
  def run() do
    Logger.info("🏛️ [REA-7E] Initiating Civilization Dividend Test (Eden Mechanics)...")
  ...
  defp evolve(pop, kg, epochs_remaining, registry, telemetry, state) do
    ...
    initial_metrics = %{
      ...
      state: state, dividend_ledger: %{}, activated_dkns: 0, total_immediate_dividends: 0.0
    }
    ...
    # 2. Apply Immediate Discovery Dividends
    foraged_pop = Enum.map(foraged_pop_raw, fn agent ->
      dividend = Map.get(epoch_metrics.dividend_ledger, agent.id, 0.0)
      %{agent | energy: agent.energy + dividend}
    end)
    ...
    # 4. Resource Regeneration & Trace Decay
    ...
    total_energy_gen = epoch_metrics.shallow_energy_gen + epoch_metrics.eden_energy_gen + epoch_metrics.total_immediate_dividends
    cd = if total_energy_gen > 0, do: (epoch_metrics.eden_energy_gen + epoch_metrics.total_immediate_dividends) / total_energy_gen, else: 0.0
    chr = epoch_metrics.activated_dkns / 100.0 # 100 DKNs out of 1000 total nodes
    
    # CROI = Energy from Eden (Commons + Dividends) / Energy from Shallow
    croi = if epoch_metrics.shallow_energy_gen > 0, do: (epoch_metrics.eden_energy_gen + epoch_metrics.total_immediate_dividends) / epoch_metrics.shallow_energy_gen, else: 99.9
    
    new_telemetry = if epochs_remaining == 1 or rem(current_epoch, 100) == 0 do
      [record_snapshot(next_gen, current_epoch, final_registry, cr, cd, chr, croi, updated_kg, final_state_with_counts) | telemetry]
    else
      telemetry
    end
    ...
    Enum.reduce(1..steps, {agent, kg, initial_m}, fn _, {current_agent, current_kg, m} ->
      ...
      cond do
        ...
        node.type == :deep_locked ->
          ...
          c_nov = if v_expl > 0.5 and node.contributors.nov == nil, do: current_agent.id, else: node.contributors.nov
          c_comp = if v_comp > 0.5 and node.contributors.comp == nil, do: current_agent.id, else: node.contributors.comp
          c_pred = if v_pred > 0.5 and node.contributors.pred == nil, do: current_agent.id, else: node.contributors.pred
          
          if c_nov != nil and c_comp != nil and c_pred != nil do
            # ACTIVATE EDEN NODE
            new_ledger = m.dividend_ledger
              |> Map.update(c_nov, @dkn_immediate_reward, & &1 + @dkn_immediate_reward)
              |> Map.update(c_comp, @dkn_immediate_reward, & &1 + @dkn_immediate_reward)
              |> Map.update(c_pred, @dkn_immediate_reward, & &1 + @dkn_immediate_reward)
        ...
        node.type == :eden ->
          ...
          updated_node = %{node | current_energy: max(0.0, node.current_energy - total_energy_gained)}
          new_kg = %{current_kg | nodes: Map.put(current_kg.nodes, target_node_id, updated_node)}
  ...
  defp regenerate_nodes(kg) do
    new_nodes = Enum.into(kg.nodes, %{}, fn {id, node} ->
      cond do
        ...
        node.type == :eden ->
          {id, %{node | 
            current_energy: min(@eden_max_energy, node.current_energy + @eden_regen_rate),
            novelty_trace: 0.0, compression_trace: 0.0, prediction_trace: 0.0
          }}
        true ->
          {id, node}
      end
  ...
  defp record_snapshot(pop, epoch, registry, cr, cd, chr, croi, kg, state) do
    pop_size = length(pop)
    ...
    active_eden_count = Enum.count(kg.nodes, fn {_, n} -> n.type == :eden end)
    
    %{
      epoch: epoch, pop_size: pop_size, diversity: diversity, active_species: active_confirmed_species,
      cr: cr, cd: cd, chr: chr, croi: croi, max_share: max_share, active_edens: active_eden_count,
      nov_res: state.novelty_reservoir,
      niche_census: nc
    }
  end
  ...
  defp print_results(telemetry) do
    Logger.info("\n=============================================================================================================================")
    Logger.info("🏛️ REA-7E CIVILIZATION DIVIDEND MATRIX (Eden Mechanics)")
    Logger.info("=============================================================================================================================")
    Logger.info("Epoch | Pop | Shannon | Spc | CR    | CD    | CROI  | Eden Nodes | Max Niche % | Nov Res | Comp | Expl | Pred | Gen")
    Logger.info("-----------------------------------------------------------------------------------------------------------------------------")
    ...
    Enum.each(Enum.reverse(telemetry), fn r ->
      ...
      croi = Float.round(r.croi, 2) |> Float.to_string() |> String.pad_trailing(5)
      edens = "#{r.active_edens}/100" |> String.pad_trailing(10)
      ...
      nc = r.niche_census
      Logger.info("#{e} | #{p} | #{d} | #{spc} | #{cr} | #{cd} | #{croi} | #{edens} | #{m_sh} | #{nr} | #{nc.comp}   | #{nc.expl}   | #{nc.pred}   | #{nc.gen}")
    end)
    Logger.info("=============================================================================================================================")
  end
...
Tiannara.REA.CivilizationDividendTest.run()
